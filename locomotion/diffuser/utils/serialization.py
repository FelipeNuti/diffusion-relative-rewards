...
import pickle
import glob
from diffuser.rrf_diffusion import TrajectoryDiscriminator
import torch
import pdb
...
def load_discriminator(
    ...
    # render_config = load_config(*loadpath, 'render_config.pkl')
    model_config = load_config(*loadpath, 'model_config.pkl')
    trainer_config = load_config(*loadpath, 'discriminator_trainer_config.pkl')
    # gradient_matching_config = load_config(*loadpath, 'gradient_matching_config.pkl')
    ...
    discriminator = TrajectoryDiscriminator(model, diffusion).to(device = "cuda:0")
    ...
    return DiffusionExperiment(dataset, None, trainer.model, diffusion, trainer.ema_model, trainer, epoch)
