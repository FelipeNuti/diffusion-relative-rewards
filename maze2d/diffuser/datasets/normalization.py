...
class DatasetNormalizer:
    ...
    def to(self, device = "cuda:0"):
        for k, v in self.normalizers.items():
            self.normalizers[k] = v.to(device = device)
        return self
...
class Normalizer:
    ...
    def __init__(self, X):
        ...
        self.mins = X.min(axis=0).values
        self.maxs = X.max(axis=0).values
    ...
    def to(self, device = "cuda:0"):
        ...
        self.mins = self.mins.to(device = device)
        self.maxs = self.maxs.to(device = device)
