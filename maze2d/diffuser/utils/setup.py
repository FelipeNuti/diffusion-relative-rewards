...
def watch(args_to_watch, wandb_run = True):
    def _fn(args):
        exp_name = []
        ...
        if wandb_run:
            exp_name.append(str(wandb.run.name))
...
class Parser(Tap):
    ...
    def mkdir(self, args):
        if 'logbase' in dir(args) and 'dataset' in dir(args) and 'dataset_size' in dir(args) and 'version' in dir(args) and 'goal' in dir(args) and 'exp_name' in dir(args):
            args.savepath = os.path.join(args.logbase, args.version, args.dataset, args.dataset_size, args.goal, args.exp_name)
