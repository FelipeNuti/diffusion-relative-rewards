...
class Parser(Tap):
    ...
    def parse_args(self, experiment=None, prepare_dirs = True):
        ...
        # args = self.patch_dataset(args, experiment)

        args.dataset_name = args.dataset.split("-")[0]
        print(f"[ utils/setup ] Set dataset_name = {args.dataset_name}")
        ...
        if prepare_dirs:
            self.prepare_dirs(args)
        return args
    
    def patch_dataset(self, args, experiment):
        triggers = ["dataset"]
        occurs = [any([f"--{trg}" in s for s in args.extra_args]) and hasattr(args, trg) for trg in triggers]
        trg = None
        for i in range(len(triggers)):
            if occurs[i]:
                ...
                break
        if trg is not None:
            base_env_names = ["halfcheetah", "hopper", "walker2d"]
            env_types = ["random", "medium-replay", "medium", "medium-expert", "expert"]

            env_names = []
            for base_env in base_env_names:
                for env_type in env_types:
                    env_names.append(f"{base_env}-{env_type}-v2")

            pattern = re.compile('|'.join(env_names))
            matches = pattern.findall(getattr(args, trg))

            assert len(matches) == 1, f"Expected name of 1 environment to occur in s_expert_path; found {matches}"

            args.dataset = matches[0]
            dataset = args.dataset.replace('-', '_')
            print(f"Detected dataset {args.dataset}")
            args = self.read_config(args, experiment)
            self.add_extras(args)
        return args

    def prepare_dirs(self, args):
    ...
    def read_config(self, args, experiment):
        ...
        for key, val in params.items():
            if key != "dim_mults" or not hasattr(self, "dim_mults") or self.dim_mults is None:
                print(f'[ utils/setup ] Loaded config | {key} : {val}')
    ...
    def add_extras(self, args):
        ...
        for v in extras:
            if "=" in v:
                for part in v.split("="):
                    separated_extras.append(part.strip())
            else:
                separated_extras.append(v)
        extras = separated_extras

        print(f'[ utils/setup ] Processed extras: {extras}\n')
    ...
    def _eval_one_fstring(self, args, key, old):
    ...
    def eval_fstrings(self, args):
        for key, old in self._dict.items():
            if type(old) is str and old[:2] == 'f:':
                new = self._eval_one_fstring(args, key, old)
    ...
    def mkdir(self, args):
        if 'logbase' in dir(args) and 'dataset' in dir(args) and 'exp_name' in dir(args):
            ...
            if 'suffix' in dir(args):
                if 'scale' in dir(args):
                    args.savepath = os.path.join(args.savepath, str(args.scale).replace('.', '-'))
