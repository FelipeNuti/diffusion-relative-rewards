"""
We modified the pipeline_stable_diffusion_safe.py file from Diffusers to facilitate
the extraction of image embeddings for relative reward function learning. This is 
our modified version of the file.
"""
    ...
    def get_deltas(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred_unsafe = noise_pred_uncond + guidance_scale * noise_guidance
                noise_pred_safe = noise_pred_uncond + guidance_scale * (noise_guidance - noise_guidance_safety)

                # compute the previous noisy sample x_t -> x_t-1
                latents_unsafe = self.scheduler.step(noise_pred_unsafe, t, latents, **extra_step_kwargs).prev_sample
                latents_safe = self.scheduler.step(noise_pred_safe, t, latents, **extra_step_kwargs).prev_sample

                if follow_safe_latent:
                    latents = latents_safe
                else:
                    latents = latents_unsafe

                # call the callback, if provided
                if i == len(timesteps) - 1 or ((i + 1) > num_warmup_steps and (i + 1) % self.scheduler.order == 0):
                    progress_bar.update()
                    if callback is not None and i % callback_steps == 0:
                        callback(i, t, latents_unsafe, latents_safe)
